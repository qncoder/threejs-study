...
// function drawDebug(A, C, B) {
//   removeSceneChildByName(DEBUG_HELPER_NAME);

//   if (!SHOW_DEBUG || !scene || typeof THREE === 'undefined') return;

//   const group = new THREE.Group();
//   group.name = DEBUG_HELPER_NAME;
//   scene.add(group);

//   const aWorld = A.clone();
//   const bWorld = B.clone();
//   const cWorld = C.clone();

//   node.updateWorldMatrix(true, true);
//   node.localToWorld(aWorld);
//   node.localToWorld(bWorld);
//   node.localToWorld(cWorld);

//   makeDebugPoint(group, 'A_green_fixed_position', 0x33cc66, aWorld, POINT_SIZE);
//   makeDebugPoint(group, 'B_red_sliding_position', 0xff3333, bWorld, POINT_SIZE);
//   makeDebugPoint(group, 'C_blue_tailbeam_position', 0x3399ff, cWorld, POINT_SIZE);

//   makeDebugLine(group, 'AB_yellow_hydraulic_length', 0xffcc00, aWorld, bWorld, LINE_WIDTH);
//   makeDebugLine(group, 'AC_red_tailbeam_side', 0xff3333, aWorld, cWorld, LINE_WIDTH);
//   makeDebugLine(group, 'BC_red_fixed_side', 0xff3333, bWorld, cWorld, LINE_WIDTH);
// }
...
if (!plateMesh || !fixedMesh || !slidingMesh) {
  ...
  const startAngle = getAngleAtC(A0, C, B);
  ...
  const targetAngle = getAngleAtC(A1, C, B);
  const targetAB = A1.distanceTo(B);
  ...
  function apply(progress) {
    ...
    // drawDebug(currentA, C, B);

    // if (progress >= 1) {
    //   const finalAngle = getAngleAtC(currentA, C, B);
    //   const finalAB = currentA.distanceTo(B);

    //   console.log('C 点，tailbeam.position，父级局部坐标:', C);
    //   console.log('A 点，tailbeam_hydraulic_fixed 跟随尾梁后的父级局部坐标:', currentA);
    //   console.log('B 点，tailbeam_hydraulic_slidingshaft 保持不动的父级局部坐标:', B);
    //   console.log('起始 ∠ACB:', startAngle);
    //   console.log('目标 ∠ACB:', targetAngle);
    //   console.log('最终 ∠ACB:', finalAngle);
    //   console.log('最终 AB 液压杆长度:', finalAB);
    //   console.log('目标 AB 液压杆长度:', targetAB);
    //   console.log('尾梁绕父级 X 轴旋转角度:', currentDelta * 180 / Math.PI);
    //   console.log('固定缸体绕 A 铰点旋转角度:', fixedDelta * 180 / Math.PI);
    //   console.log('活动杆绕 B 铰点旋转角度:', slidingDelta * 180 / Math.PI);
    // }
  }
