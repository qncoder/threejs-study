function run() {
  ...
  const STORE_KEY = '__tailBeamRunAnimationStore'
  ...
  window.stopTailBeamRunAnimation = stopOldAnimation

  const AObject = node.getObjectByName('tailbeam_pos')
  const BObject = node.getObjectByName('tailbeam_hydraulic_fixed_pos')
  const CObject = node.getObjectByName('tailbeam_hydraulic_slidingshaft_pos')

  if (!AObject || !BObject || !CObject) {
    console.warn(
      '尾梁动画缺少节点，请检查 tailbeam_pos、tailbeam_hydraulic_fixed_pos、tailbeam_hydraulic_slidingshaft_pos'
    )
    return
  }
