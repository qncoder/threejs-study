...
if (FORCE_INIT || !window[STORE_KEY][sceneKey]) {
  window[STORE_KEY][sceneKey] = {
    group: new HydraulicMechanism(scene),
    step: 40,
  }
  console.log('已初始化 HydraulicMechanism 实例')
} else {
  window[STORE_KEY][sceneKey].step += 1
  console.log('复用已有 HydraulicMechanism 实例')
}

const state = window[STORE_KEY][sceneKey]
console.log('当前 step:', state.step)
state.group.rodTopBeamAngleToHydraulicLength(state.step, 0)
